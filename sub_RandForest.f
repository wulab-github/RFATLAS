	subroutine RandForest(mdim,ntrain,nclass,
     &            ntest,x2,cl2,xts2,clts2,
     &            jests2,qts2,
     &            labelts,labeltr,
...
c	ALL RELEVANT OUTPUT FILES MUST BE GIVEN NAMES--SEE BELOW.
	...
	real*8 x2(mdim,ntrain),xts2(mdim,ntest)
	integer cl2(ntrain),clts2(ntest)
	integer jests2(ntest)
	real*8 qts2(nclass,ntest)
c
...
c
	parameter(
c  		DESCRIBE DATA		
c     1		mdim=2,ntrain=10,nclass=2,
     1   	maxcat=1,
c     1		ntest=4,labelts=1,labeltr=1,
c
...
c
	...
	parameter(
     &		isumout	= 	0,!0/1		1=summary to screen
     ...
     &		iproxout=	0,!0/1/2	1=prox,2=adds test(13)
...
c
	integer nsample,nrnodes,mimp,near,
     &	ifprot,ifscale,iftest,mdim0,ntest0,nprot0,nscale0
c	parameter(
c     &  nsample=(2-labeltr)*ntrain,
c     &  nrnodes=2*nsample+1,
c     &  mimp=imp*(mdim-1)+1,
c     &  ifprot=nprot/(nprot-.1),
c     &  ifscale=nscale/(nscale-.1),
c     &  iftest=ntest/(ntest-.1),
c     &  nprot0=(1-ifprot)+nprot,
c     &  nscale0=(1-ifscale)+nscale,
c     &  ntest0=(1-iftest)+ntest,
c     &  mdim0=interact*(mdim-1)+1,
c     &  near=nprox*(nsample-1)+1)
C
c	-------------------------------------------------------
...
c
c	if (idataout.ge.1)
c     &		open(7,file='save-data-from-run',status='new')
	if (impfastout.eq.1)
...
c
c	open(16, file='data.train', status='old')
c	do n=1,ntrain
c		read(16,*) (x(m,n),m=1,mdim),cl(n)
c	enddo
c	close(16)
c	if(ntest.gt.0) then
c		open(17, file='data.test', status='old')
c		if(labelts.ne.0) then
c			do n=1,ntest0
c				read(17,*) (xts(m,n),m=1,mdim),clts(n)
c			enddo
c		else
c			do n=1,ntest0
c				read(17,*) (xts(m,n),m=1,mdim)
c			enddo
c		endif
c		close(17)
c	endif
...
c		write(*,*) 'class counts-training data'
c		write(*,*) (nc(j),j=1,nclass)
		if(ntest.gt.0.and.labelts.eq.1) then
c			print*
c			write(*,*) 'class counts-test data'
c			write(*,*) (ncts(j),j=1,nclass)
		endif
	endif
...
c			write(*,'(i8,100f10.2)') 
c    &			jb,100*errtr,(100*tmiss(j),j=1,nclass)
		else
c			write(*,'(i8,2f10.2)') jb,100*errtr
		endif
		endif
		if(ntest.gt.0) then
     ...
     & 			tmissts,ncts,jests,labelts)
			...
			if(labelts.eq.1) then
			   if(lookcls.eq.1) then
c				write(*,'(i8,20f10.2)') jb,100*errts,
c     &				(100*tmissts(j),j=1,nclass)
			   else
c				write(*,'(i8,2f10.2)') jb,100*errts
			   endif
c			   print *
			endif
			endif
...
c
	if(idataout.ge.1) then	
        do n=1,nsample   
c	   print*,n,cl(n),jest(n),
c     & 			(q(j,n),j=1,nclass),(x(m,n),m=1,mdim)
c		do n=1,nsample
c			write(7,'(3i5,5000f10.3)') n,cl(n),jest(n),
c     & 			(q(j,n),j=1,nclass),(x(m,n),m=1,mdim)
		enddo
	endif
	if(idataout.eq.2.and.ntest.gt.0) then
c			do n=1,ntest0
c				print*, n,clts(n),jests(n),
c     & 				(qts(j,n),j=1,nclass),(xts(m,n),m=1,mdim)
c			     enddo
	   do n=1,ntest0
	      jests2(n)=jests(n)
	      do j=1,nclass
	      ...
	      enddo
	   enddo
c		if(labelts.eq.1) then
c			do n=1,ntest0
c				write(7,'(3i5,1000f10.3)') n,clts(n),jests(n),
c     & 				(qts(j,n),j=1,nclass),(xts(m,n),m=1,mdim)
c			enddo
c		else
c			do n=1,ntest0
c				write(7,'(2i5,1000f10.3)') n,jests(n),
c     & 				(qts(j,n),j=1,nclass),(xts(m,n),m=1,mdim)
c			enddo
c		endif
	endif
     ...
     & 			(q(j,n),j=1,nclass)
		...
		enddo
		    if (iprotout.eq.1) then
		 		do j=1,nclass
		 			do i=1,npend(j)
					write(26,'(3i5,5000f10.3)') j,i,1,
     &		          			(protv(msm(m),i,j),m=1,mdimt)
					write(26,'(3i5,5000f10.3)') j,i,2,
     &	          			     (protvlow(msm(m),i,j),m=1,mdimt)
					write(26,'(3i5,5000f10.3)') j,i,3,
     &         				     (protvhigh(msm(m),i,j),m=1,mdimt)
